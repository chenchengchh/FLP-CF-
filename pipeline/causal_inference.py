...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        # Step 4: Decode the output (chunked to bound peak GPU memory on 24GB)
        DECODE_CHUNK = 32
        num_out_frames = output.shape[1]
        chunks = []
        for s in range(0, num_out_frames, DECODE_CHUNK):
            e = min(s + DECODE_CHUNK, num_out_frames)
            vchunk = self.vae.decode_to_pixel(output[:, s:e], use_cache=False)
            vchunk = (vchunk * 0.5 + 0.5).clamp(0, 1)
