...
# Create dataset
_res_h = int(os.environ.get("CF_RES_H", 480))
_res_w = int(os.environ.get("CF_RES_W", 832))
if args.i2v:
    assert not dist.is_initialized(), "I2V does not support distributed inference yet"
    transform = transforms.Compose([
        transforms.Resize((_res_h, _res_w)),
        transforms.ToTensor(),
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    if args.i2v:
        ...
        sampled_noise = torch.randn(
            [1, args.num_output_frames - 1, 16, _res_h // 8, _res_w // 8], device=device, dtype=torch.bfloat16
        )
    else:
        ...
        initial_latent = None
        sampled_noise = torch.randn(
            [1, args.num_output_frames, 16, _res_h // 8, _res_w // 8], device=device, dtype=torch.bfloat16
        )
